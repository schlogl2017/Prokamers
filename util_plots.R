...
hist.by.class <- function (x,
  ...
  legend(leg.pos[1], leg.pos[2], legend=c(groups),col=(1:length(groups))+1,lwd=3, bg='white', bty='o')
}
...
plot.profiles <- function(x,			# data frame
  ...
  message.verbose("Plot profiles", 3)
  message.verbose(dim(x),3)
  message.verbose("Selection", 3)
  message.verbose(length(selection), 3)
  message.verbose(selection, 4)
  par(las=las)
...
plot.frequency.distrib <- function(x,
                                   type="frequency", ## Values to be plotted on Y axis. Supported: freq (default), counts, density, cdf, dcdf
                                   main="",
...
#                                   cumul=FALSE,
  ...
  ## Define column-specific colors
  if (is.na(col)) {
    colors <- rainbow(n=col.number)
  } else {
    colors <- rep(col,length.out=col.number)
  }

  ## Calculate class frequencies
  breaks <- pretty(as.vector(as.matrix(x)), n=class.number+1)
  class.number <- length(breaks)-1
  message.verbose(paste("classes", class.number))
#  curves <- matrix(nrow=class.number,ncol=ncol(x))
#  message.verbose(paste("dim(curves)", dim(curves)))
  ...
  for (i in 1:col.number) {
    ...
    if (tolower(type) == "cdf") {
      ...
      values <- cumsum(h$counts/sum(h$counts))
    } else if (tolower(type) == "dcdf") {
      ## Decreasing cumulative density function
      freq <- h$counts/sum(h$counts)
      values <- rev(cumsum(rev(freq)))
#      values <- 1 - cumsum(h$counts/sum(h$counts))
#      values <- c(1, values[1:(length(values)-1)])
    } else if (tolower(type) == "density") {
      ## Density
      values <- h$density
    } else if (tolower(type) == "counts") {
      ## Counts
      values <- h$counts
    } else if (tolower(type) == "frequency") {
      ## Compute frequencies
      values <- h$counts/sum(h$counts)
    } else {
      stop(paste(type, "is not a valid type for plot.frequency.distrib"))
    }
    curves <- rbind(curves, values)
  }
  curves <- as.data.frame(t(curves))
  row.names(curves) <- mids

  ## Calculate Y limits
  if (is.na(ylim)) {
    if (length(grep (pattern="cdf",tolower(type))) > 0) {
      ylim <- c(0,1)
    } else {
  ...
  if (single.plot) {
    ...
    ## Start the plot with the first distribution
    plot(mids, curves[,1],
         type="l",
         lty=lty[1],
         xlim=xlim,
...
#         font.lab=2,
    ...
    ## Add lines with  the other distributions
    ...
    ## draw the legend
    if (plot.legend) {
      legend(legend.location, legend=colnames, col=colors,
             lwd=lwd,cex=0.8,bg='white', bty='o', lty=lty)
    }
...
#           font.lab=2,
  ...
  ## Draw vertical line at the modal value
  if (draw.mode) {
...
################################################################
##
## Plot density distributions from a data frame.
## One polygon frequency per column.
##
## This function calls the more general function
## plot.frequency.distrib().
plot.density.distrib <- function(x, type="density", ...) {
  plot.frequency.distrib(x, type="density", ...)
}
...
plot.prcomp.groups <- function(x, ### A data frame with the multivariate data
	...
	message.verbose("Calculating principal components on the labeled objects only, using correlation matrix", 2)
	pc <- prcomp(x[!is.na(group.labels),])
        message.verbose("Rotating and scaling the whole data set", 2)
        if (scale.axes) {
    ...
    legend(xleg,yleg,groups,col=group.palette[groups],pch=19,cex=1, bg="white", bty="o")
    rm(group.labels.tmp)
    if (return.pc) {
	return(pc)
    }
